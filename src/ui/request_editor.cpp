...
// KV 原语（KvTable/认证桥接/类型推断等）再拆至 request_editor_kv.cpp，经 ui.h 声明复用。
...
// environment_widgets.cpp 的同名桥接同逻辑，P1-C3 归并唯一实现。
...
// InferKvType 定义在 request_editor_kv.cpp（KV 原语 owner）；普通头不声明纯
// helpers（ui.h 模块约束惯例），本 TU 单点调用走文件内前向声明。
