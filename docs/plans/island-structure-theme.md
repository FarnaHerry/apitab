...
实机回归待人工，见 §13.6 后"P1-B0.4 收口结果"）；P1-C1/P1-C2 已完成，P1-C3
公共声明整理进行中；下一批 P1-B1「菜单统一与项目上下文条」待启动；P2 待继续**。
...
P1-C 当前状态（2026-09-02）：

- **P1-C1 已完成**：`request_page.cpp` 已由约 2897 行收束为 426 行薄编排层；请求
  编辑器、请求集合树、标签条、环境控件、Body 编辑器、响应区和文档页均已按功能域
  分出，最大拆出文件 `request_editor.cpp` 为 847 行。
- **P1-C2 已完成**：`app.cpp` 已由约 1561 行收束为 670 行应用壳编排层；标题栏、
  底部状态条和应用级对话框分别位于 `title_bar.cpp`、`global_status_bar.cpp`、
  `app_dialogs.cpp`。
- **P1-C3 进行中**：待整理 `ui.h` 中跨 TU 声明和 request 相关桥接 helper；目标是
  按职责拆出局部头文件、归并可安全共享的 KV/类型推断逻辑，同时保持模块头文件墙、
  source-first 构建和现有 UI 行为不变。
...
```text
窗口海面 Ocean
├── 顶部导航条 Top Dock（不做岛，直接落海面）
│   ├── Logo
│   ├── 主页与项目标签
│   └── 全局操作
├── 工作区
│   ├── 项目工具栏 Tool Rail（不做岛，直接落海面）
│   └── 页面画布
│       ├── 页面标题区
│       ├── 主工作岛 Primary Island
...
- 顶部导航区（标题栏）与左侧项目工具栏**不做岛**（2026-09-13 调整，此前为导航岛/
  窄工具岛）：壳层导航直接落在海面（窗口背景）上，与业务岛拉开层级——只有业务
  内容区域成岛；选中态用实心图标变体和左侧指示条强调，不依赖岛背景。活动项目
  标签可在条内抬升，非活动标签不各自成卡。
...
- **P1-C2（app.cpp 拆分）**：已完成（并行 agent `p1c2-app`），记录见 §16.6。

### 16.6 P1-C2 执行记录（2026-09-02）：应用壳拆分 → title_bar / global_status_bar / app_dialogs
...
- **留给 P1-C3**：
  ...
  - P1-C1/P1-C2 已在后续合并提交中完成；当前仅保留 P1-C3 公共声明整理，以及可选的
    `SideShell`/主题派生进一步拆分评估。

### 16.7 P1-C 状态复核（2026-09-02）

对照最新提交 `eb622ca`（P1-C2 合并）及当前工作树复核：P1-C1、P1-C2 的代码和文档
记录均已落地，未发现仍需执行的“右岛剩余功能域”或“标题栏/状态条/对话框抽离”任务。
当前 `src/ui` 中没有超过 1000 行的业务实现文件；最大文件为 `request_editor.cpp`
（847 行），其次为 `common.cpp`（824 行）和 `request_list.cpp`（764 行）。因此不应
再为满足行数指标继续拆分。

P1-C3 是当前唯一进行中的结构整理任务：重点检查 `ui.h` 的跨 TU 声明是否应迁移到
职责清晰的局部头文件，并评估 `ToKeyValue`/`FromKeyValue`/`InferKvType` 等桥接逻辑
是否可以在不违反模块头文件墙的前提下归并。完成后须更新本节状态并重新执行构建、
定向测试和 `git diff --check`。
